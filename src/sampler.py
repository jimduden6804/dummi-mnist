...
def sample_index(prob, rand):
    return np.where(np.less_equal(rand(), np.cumsum(prob)))[0][0]
...
def generate_example(anchor_prob=ANCHOR_PROB,
    ...
    print(click_prob)
    click_position = sample_index(click_prob, rand) #rand gibt error, weil probs nicht normalisiert
    # click_position = sample(click_prob, rand)

    return {'reco': reco, 'click_position': click_position}
